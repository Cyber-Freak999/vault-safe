...
Gate note (integration gap found in planning, corrected): the server has NO global `GET /api/items` list route — item listing is vault-scoped by design (`GET /api/vaults/<vault_id>/items`). The client's `list_items()` without `vault_id` would 404; the integration test therefore passes `vault_id=`. The client's no-vault branch is a latent dead path to be resolved in the final whole-branch review. `live_server` runs in-process and shares the default (LocMem) cache with the test runner; DRF's `SimpleRateThrottle` cache key is rate-independent, so cross-test login history throttles other tests. Tests therefore clear the shared cache in an autouse fixture (`_isolate_throttle_cache` in `server/tests/conftest.py`). Deviation in Step 3 (implemented): `override_settings(REST_FRAMEWORK=...)` cannot change the throttle rate once `rest_framework.throttling` has been imported (DRF freezes `SimpleRateThrottle.THROTTLE_RATES` at first import), making the test order-dependent. The throttle test instead patches `SimpleRateThrottle.THROTTLE_RATES = {**rates, "login": "3/min"}` for the duration of the test and restores it in a `finally`.
